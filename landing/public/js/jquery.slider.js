...
(function ($) {
  ...
  jSlider.prototype.init = function (node, settings) {
    ...
    if (
      ...
      $.isFunction(this.settings.onstatechange)
    )
  ...
  jSlider.prototype.create = function () {
    ...
    $.extend(this.o, {
      ...
      labels: {
        0: {
          o: this.domNode
            .find(OPTIONS.selector + "value")
            .not(OPTIONS.selector + "value-to"),
        },
        1: {
          o: this.domNode
            .find(OPTIONS.selector + "value")
            .filter(OPTIONS.selector + "value-to"),
        },
      },
      limits: {
        0: this.domNode
          .find(OPTIONS.selector + "label")
          .not(OPTIONS.selector + "label-to"),
        1: this.domNode
          .find(OPTIONS.selector + "label")
          .filter(OPTIONS.selector + "label-to"),
      },
    });
  ...
  jSlider.prototype.redraw = function (pointer) {
    ...
    if (this.o.pointers[0] && this.o.pointers[1])
      this.o.value.css({
        ...
        width:
          this.o.pointers[1].value.prc - this.o.pointers[0].value.prc + "%",
      });
    ...
    if (!this.settings.single) {
      ...
      switch (pointer.uid) {
        ...
        case 1:
          if (
            sizes.border - sizes.label / 2 <
            another_label.o.offset().left -
              this.sizes.domOffset.left +
              another_label.o.outerWidth()
          ) {
            another_label.o.css({ visibility: "hidden" });
            another_label.value.html(this.nice(another.value.origin));
    ...
    if (this.settings.limits) {
      ...
      for (key in this.o.pointers) {
        if (!this.settings.single || key == 0) {
          ...
          if (
            label_left + label.o.outerWidth() >
            this.sizes.domWidth - limit.outerWidth()
          )
            limits[1] = false;
        }
      }
    ...
    if (this.settings.heterogeneity && this.settings.heterogeneity.length > 0) {
      ...
      for (var i = 0; i <= h.length; i++) {
        ...
        if (prc >= _start && prc <= v[0]) {
          var value =
            _from + ((prc - _start) * (v[1] - _from)) / (v[0] - _start);
        }
  ...
  jSlider.prototype.round = function (value) {
    ...
    if (this.settings.round)
      value =
        Math.round(value * Math.pow(10, this.settings.round)) /
  ...
  jSliderPointer.prototype.onmouseup = function (evt) {
    if (
      ...
      $.isFunction(this.parent.settings.callback)
    )
